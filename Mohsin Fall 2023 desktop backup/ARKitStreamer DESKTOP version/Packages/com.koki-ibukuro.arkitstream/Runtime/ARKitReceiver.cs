...
namespace ARKitStream
{
    ...
    public sealed class ARKitReceiver : MonoBehaviour
    {
        ...
        public Texture2D DepthTexture => texture2Ds == null ? null : texture2Ds[2];
        public Texture2D StencilTexture => texture2Ds == null ? null : texture2Ds[3];

        // private Texture2D dstRT;
        ...
        private void Start()
        {
            ...
            // RGBMaterial = new Material(Shader.Find("Unlit/ARKitStreamReceiverRGB"));
            SetupPose();
            ...
            // CbCrTexture = new Texture2D(width, height, TextureFormat.RGB24, false)
            // {
            //     filterMode = FilterMode.Point
            // };
   ...
   int numParticles = ndiSourceSize.x * ndiSourceSize.y;
        ...
        private void OnGUI()
        {
            ...
            // RGBMaterial.ReadPixels(new Rect(0, 0, width, height), 0, 0);
            // pngBytes = tmp.EncodeToPNG();
            // GUI.DrawTexture(new Rect(255, 0, 256, 256), RGBMaterial.GetTexture("_MainTex") as Texture2D);
            // GUI.DrawTexture(new Rect(0, 256, 256, 256), CbCrTexture);
            var numoftextures = texture2Ds.Length;
            var pictureSize = 64;
            Debug.Log(numoftextures);
            for (int i = 0; i < numoftextures; i++)
            {
                GUI.DrawTexture(new Rect(0, pictureSize * i , pictureSize, pictureSize * i  + pictureSize), texture2Ds[i]);
            }
